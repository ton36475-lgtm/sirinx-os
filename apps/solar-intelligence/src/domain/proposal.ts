...
export function createProposal(
  ...
  return {
    ...
    executiveSummary: [
      ...
      `Estimated monthly savings are THB ${roi.monthlySavingsThb.value.toLocaleString()} with simple payback around ${roi.simplePaybackYears.value} years.`,
      `Backup plan: ${backupLine}.`,
      blockers.length
        ? `Do not issue final quote until ${blockers.length} blocker(s) are resolved.`
        : "No blocking compatibility issue is present in the current rule set."
    ].join(" "),
    billOfMaterials: [
      {
    ...
    thailandComplianceSummary: design.compatibility.filter((check) =>
