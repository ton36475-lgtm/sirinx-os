#!/usr/bin/env node
/**
 * scripts/ocr-qa-workflow.mjs — Automated OCR → QA Gate pipeline
 *
 * Flow:
...
function runGate(target) {
  try {
    ...
    // gate อาจ exit 1 (REJECTED) — stdout ยังมี JSON
...
if (!input) {
  console.error("Usage: node scripts/ocr-qa-workflow.mjs <pdf|image|code-file>");
  process.exit(2);
}

console.log(`[OCR-QA] เริม่ pipeline สำหรับ: ${input}`);
