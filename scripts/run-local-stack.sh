...
LOG_DIR="${SIRINX_LOG_DIR:-$ROOT/ops/logs}"
PID_DIR="${SIRINX_PID_DIR:-$ROOT/ops/pids}"

API_URL="${DEV_CONTROL_API_URL:-http://127.0.0.1:8711/health}"
DASHBOARD_URL="${DEV_DASHBOARD_URL:-http://127.0.0.1:8710}"
SOLAR_URL="${SOLAR_INTEL_URL:-http://127.0.0.1:8720/health}"
SITE_URL="${SIRINX_SITE_URL:-http://127.0.0.1:8730}"
...
start_stack() {
  ...
  start_detached "solar-intelligence" "$SOLAR_URL" "$PID_DIR/solar-intelligence.pid" "$LOG_DIR/solar-intelligence.log" pnpm solar:dev
  start_detached "sirinx-site" "$SITE_URL" "$PID_DIR/sirinx-site.pid" "$LOG_DIR/sirinx-site.log" pnpm site:dev

  wait_for "dev-control-api" "$API_URL"
  wait_for "dev-dashboard" "$DASHBOARD_URL"
  wait_for "solar-intelligence" "$SOLAR_URL"
  wait_for "sirinx-site" "$SITE_URL"

  echo "dev dashboard: $DASHBOARD_URL"
  echo "solar intelligence: ${SOLAR_INTEL_HOME_URL:-http://127.0.0.1:8720}"
  echo "sirinx.co local site: $SITE_URL"
}

stop_stack() {
  stop_pid "sirinx-site" "$PID_DIR/sirinx-site.pid"
  stop_pid "solar-intelligence" "$PID_DIR/solar-intelligence.pid"
...
status_stack() {
  status_one "dev-control-api" "$API_URL"
  status_one "dev-dashboard" "$DASHBOARD_URL"
  status_one "solar-intelligence" "$SOLAR_URL"
  status_one "sirinx-site" "$SITE_URL"
}
...
foreground_stack() {
  ...
  if ! is_up "$API_URL"; then
    (
      cd "$ROOT"
      pnpm dev:api
    ) >"$LOG_DIR/dev-control-api.foreground.log" 2>&1 &
    owned_pids+=("$!")
  fi
  ...
  if ! is_up "$DASHBOARD_URL"; then
    (
      cd "$ROOT"
      pnpm dev:dashboard
    ) >"$LOG_DIR/dev-dashboard.foreground.log" 2>&1 &
    owned_pids+=("$!")
  fi

  if ! is_up "$SOLAR_URL"; then
    (
      cd "$ROOT"
      pnpm solar:dev
    ) >"$LOG_DIR/solar-intelligence.foreground.log" 2>&1 &
    owned_pids+=("$!")
  fi

  if ! is_up "$SITE_URL"; then
    (
      cd "$ROOT"
      pnpm site:dev
    ) >"$LOG_DIR/sirinx-site.foreground.log" 2>&1 &
    owned_pids+=("$!")
  fi
  ...
  while true; do
    for pid in "${owned_pids[@]}"; do
      if ! kill -0 "$pid" >/dev/null 2>&1; then
        echo "local stack process exited: $pid" >&2
        exit 1
      fi
    done
    sleep 2
...
case "${1:-start}" in
  ...
  open)
    start_stack
    open "$DASHBOARD_URL"
    open "${SOLAR_INTEL_HOME_URL:-http://127.0.0.1:8720}"
    open "$SITE_URL"
    ;;
  foreground)
    foreground_stack
