#!/usr/bin/env node
/**
 * scripts/qa-gatekeeper.js — OmniRoute QA Gatekeeper (รุ่น 1, ใช้ของจริง)
 *
 * ตามสเปค "OmniRoute QA Gatekeeper Protocol" ที่ผู้ใช้ให้มา:
 *   L1 Pre-Flight   : external-gate-readiness.sh (fail-soft)
...
export function runGate(targetFile) {
  ...
  if (failed.length === 0) {
    ...
    REPORT.mandate = {
      ...
      fix_command: `node scripts/qa-gatekeeper.js --fix ${targetFile}`,
    };
  }
  return REPORT;
}
...
// CLI
const target = process.argv.find((a) => a.endsWith(".js") || a.endsWith(".py") || a.endsWith(".ts")) || `${ROOT}/scripts`;
