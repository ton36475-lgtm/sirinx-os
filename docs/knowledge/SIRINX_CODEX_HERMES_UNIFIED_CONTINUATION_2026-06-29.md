...
2. Use `data/pathspecs/sirinx_codex_hermes_execution_queue_2026-06-29.json` as the ordered local queue.
3. Hermes may review `data/pathspecs/ghostclaw_lane1_packet013_decision_draft_2026-06-29.json` as draft-only aid.
4. Hermes or the operator starts/verifies `127.0.0.1:9000`, then Hermes records a separate decision for `packet_013`: `route_to_opus`, `request_revision`, `open_codex_recorder_gate`, or `block`.
5. Use `docs/knowledge/SIRINX_GHOSTCLAW_LANE1_HERMES_DECISION_INTAKE_HANDOFF_2026-06-29.md` for the exact decision path, required fields, local evidence list, and validation commands.
6. Hermes/Opus may use `data/pathspecs/ghostclaw_lane1_opus_authoring_bundle_2026-06-29.json` as a compact authoring input; it is not the final packet.
7. After a separate final Opus packet exists, run `WORKSPACE_SCAFFOLD/scripts/validate_lane1_opus_architecture_packet.py` before treating it as completion evidence.
8. Rerun `WORKSPACE_SCAFFOLD/scripts/build_lane1_hermes_decision_transition_guard.py` against that separate decision artifact before any recorder-gate, Opus-packet, or LANE_2 state change.
9. Run or inspect `WORKSPACE_SCAFFOLD/scripts/probe_active_goal_blockers.py` before refreshing blocker snapshots from older evidence.
