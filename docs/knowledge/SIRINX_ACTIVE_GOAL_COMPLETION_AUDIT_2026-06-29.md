...
| Codex/Hermes A2A queue status | `data/pathspecs/sirinx_codex_hermes_a2a_queue_status_2026-06-29.json`, `docs/knowledge/SIRINX_CODEX_HERMES_A2A_QUEUE_STATUS_2026-06-29.md`, `WORKSPACE_SCAFFOLD/reports/codex_hermes_a2a_queue_status_latest_2026-06-29.json` | Local file-bus indexed; not executed; `packet_024` is a local `/goal` inbox command, `packet_025` is Browser Use candidate outbox evidence, packet_026 through packet_040 are visible as local outbox evidence, `packet_013` remains current actionable, Hermes packet_013 decision is recorded, final LANE_1 packet is still absent, counts are `inbox=5 outbox=34 working=1 done=8 blocked=0 total=48` | Use as current queue state evidence only; it does not import all chats, execute queue items, install or run Browser Use, authorize LANE_2, execute runtime work, deploy, push, or claim MIT licensing while no root LICENSE exists |
| packet_032 local A2A sync receipt | `docs/knowledge/SIRINX_GHOSTCLAW_LANE1_PACKET032_A2A_SYNC_RECEIPT_2026-07-02.md`, `.ghostclaw_runtime/a2a2a/receipts/queue_coordination_20260701T231950_334409Z.json`, `.ghostclaw_runtime/a2a2a/receipts/hermes_route_queue_coord_packet_032_hermes_20260701T231950_334409Z.json`, `.ghostclaw_runtime/a2a2a/receipts/kob_verdict_queue_coord_packet_032_kob_20260701T231950_334409Z.json` | Local A2A sync evidence exists; final packet still missing | Supporting evidence only; it does not prove provider/Opus execution, create the final packet, mutate state, authorize LANE_2, or open external gates |
...
| Night Watch fresh status | `.hermes/logs/night-watch-latest.md` is fresh and reports WARN with local stack services offline while Hermes Desktop/gateway and public website probes are online | Incomplete | No service repair or restart without approval; do not run install/purge/stack restart workaround without an exact gate |
...
2. Treat `data/pathspecs/ghostclaw_lane1_packet013_decision_draft_2026-06-29.json`
   as historical draft-only aid superseded by the recorded `packet_026`
   `route_to_opus` decision.
3. Hermes or the operator starts/verifies `127.0.0.1:9000` separately before
   any live routing claim; the local decision receipt remains file-bus evidence.
...
5. Use `WORKSPACE_SCAFFOLD/scripts/build_lane1_hermes_decision_transition_guard.py`
   output as local evidence that `route_to_opus` maps to
   `await_opus_architecture_packet`; do not mutate state, create the final
   packet, or authorize LANE_2 from the guard.
