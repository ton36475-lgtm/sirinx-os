...
| `REQ-CODEX-HERMES-AUTONOMY` | Partial | `local_file_bus_only_runtime_blocked` | Local A2A inbox/outbox packets, queue, packet_022 next-safe-action sequencer, and packet_023 gateway current recheck | Hermes decision artifact and gateway proof or approved local alternative | Review packet_023 and packet_022, then keep file-bus packets local until Hermes records a separate packet_013 decision |
