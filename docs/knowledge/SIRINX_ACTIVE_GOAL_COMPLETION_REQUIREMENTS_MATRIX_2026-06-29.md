...
| `REQ-CODEX-HERMES-AUTONOMY` | Partial | `hermes_decision_recorded_runtime_still_blocked` | Local A2A inbox/outbox packets, queue, Hermes packet_013 decision is recorded, packet_022 next-safe-action sequencer, packet_023 gateway current recheck, and packet_024 sync-all-jobs goal command | Gateway proof, final LANE_1 packet evidence, or approved local alternative | Keep file-bus packets local while final LANE_1 packet and gateway proof remain missing |
| `REQ-LANE1-FINAL-PACKET` | Blocked | `blocked_missing_final_opus_packet` | Hermes packet_013 decision is recorded, draft packet, packet 013 readiness, decision validator, Opus packet gate validator, Opus authoring bundle | Final Opus packet plus Opus packet gate validation | Hermes/Opus may use packet_019 as authoring input, then validate the separate final packet with the Opus packet gate |
