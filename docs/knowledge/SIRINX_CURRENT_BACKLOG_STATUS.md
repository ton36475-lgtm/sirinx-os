...
| External gate approval packets | Superseded by current 4-gate set | Older 9-gate release flow was completed or replaced; current work now tracks Codex Mobile, Telegram/LINE, Solis, and Cloudflare Bot Management only |
| External gate audit preflight | Superseded by current 4-gate set | Older 9-gate release preflight was completed or replaced; current preflight keeps external execution blocked until exact human/credential gates are satisfied |
...
| Command Center current external-gate packet refresh | Done locally | `/api/external-gate-packets` and `/api/external-gate-preflight` now expose only the 4 remaining external gates; local smoke on port `8791` returned 4 packets, 4 preflight entries, `canExecuteNow=false`, and `externalWrites=false` |
...
| External gate evidence intake kit | Done locally | `docs/knowledge/external-gates/` contains templates plus `pnpm external-gates:evidence-check`; current state includes the 4 operational gates plus the separate `sirinx-os` GitHub publish gate, with `ready=0` and `unsafe=0` because no human/credential/target evidence has been supplied yet |
| External gate runner readiness | Done locally | `GET /api/external-gate-runner` and `pnpm external-gates:runner` list safe local checks and blocked external actions for all 5 evidence gates with `canExecuteNow=false` and `externalWrites=false` |
...
| GitHub bounded extraction workstreams | Done locally | `/api/github-integration` now includes 7 extraction tasks across solar ops, agent runtime, marketing/CRM, Cloudflare worker review, and mobile sensitive-file gates; docs added for solar ops extraction, agent runtime extraction, and marketing/CRM schema comparison |
| Lead qualification v2 | Done locally | `services/dev-control-api/src/lead-qualification.mjs` now exposes UTM attribution, deterministic reasons, risk flags, traffic status, and SIRINX solar segments with `externalWrites=false`; unit tests cover 5 cases |

## Current Truth

- `www.sirinx.co` remains protected as the public Solar company website.
- `sirinx-os` is on branch `codex/urgent-backlog-execution` with Command Center local-only workflow phases committed through refreshed external gate audit preflight and backlog status.
...
- GitHub extraction truth: the first executable integration layer is now a controlled workstream map, not a runtime import. `sirinx-solar-energy` contributes solar entity and ROI cross-check candidates, `oz-corp-omega-dual-node` contributes safe-command/memory-policy references, marketing repos contribute lead/CRM schema ideas, Cloudflare worker code remains deploy-gated, and mobile repos remain sensitive-file blocked.
- Lead qualification v2 truth: old marketing/CRM scoring ideas were adapted into the current local lead model as UTM attribution, deterministic reasons, risk flags, traffic status, and solar-specific segments. This is not a CRM write, customer message, production POST, or quote.
- Current external-gate preflight after PageSpeed release: Command Center tracks exactly 4 operational external gates: Codex Mobile QR/MFA, Telegram/LINE recipient/token setup, Solis read-only telemetry consent/credential/station mapping, and Cloudflare Bot Management official review. The evidence/runner layer also tracks the separate `sirinx-os` GitHub publish target. Hermes profiles exist for `shogun`, `planner`, `frontend`, `backend`, `devops`, `qa`, `growth`, `sales`, `data`, `solis`, `design`, and `scribe`; default gateway is running; Hermes pairing list has no pending pairings; Telegram is configured but not proven deliverable; LINE and Solis remain credential/consent blocked.
...
10. Surface lead-quality v2 reasons/risk flags more clearly in Command Center, then design local lead-event/audit records before any CRM write.
