"""Live A2A queue visibility guard for packet_031."""
...
PACKET = ROOT / "_A2A_QUEUE" / "outbox" / "packet_031_sirinx_coding_engine_security_rules_work_report_draft.json"
DOC = ROOT / "docs" / "knowledge" / "SIRINX_CODING_ENGINE_SECURITY_RULES_WORK_REPORT_A2A_VISIBILITY_2026-07-02.md"
...
class CodingEngineSecurityRulesWorkReportA2AVisibilityTests(unittest.TestCase):
    """Ensure packet_031 is indexed as a Telegram draft without execution."""

    def test_packet_031_is_visible_in_live_queue_without_execution(self):
        module = load_builder()
        snapshot = module.build_queue_status(queue_root=ROOT / "_A2A_QUEUE", root=ROOT)
        packet = next(item for item in snapshot["packets"] if item["id"] == "packet_031")
        ...
        self.assertEqual(packet["path"], "_A2A_QUEUE/outbox/packet_031_sirinx_coding_engine_security_rules_work_report_draft.json")
    ...
    def test_raw_packet_keeps_report_and_execution_gates_closed(self):
