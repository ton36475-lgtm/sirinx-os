export type XAccountSeed = {
  handle: `@${string}`;
  sourceUrl: string;
  internalUse: string;
  verificationStatus: "user-seed" | "needs-review";
  automationAllowed: false;
  evidenceNote: "Evidence missing — source verification required";
  notes: string;
};

export const X_AI_ACCOUNTS: readonly XAccountSeed[] = Object.freeze([
...
export function validateXAccounts(accounts: readonly XAccountSeed[] = X_AI_ACCOUNTS) {
  ...
  for (const account of accounts) {
    ...
    seen.add(key);
    if (!["user-seed", "needs-review"].includes(account.verificationStatus)) {
      findings.push(`invalid_verification_status:${account.handle}`);
    }
    if (account.automationAllowed !== false) {
      findings.push(`automation_must_remain_false:${account.handle}`);
    }
    if (account.evidenceNote !== "Evidence missing — source verification required") {
      findings.push(`evidence_note_required:${account.handle}`);
    }
...
function creator(handle: `@${string}`, internalUse: string): Readonly<XAccountSeed> {
  return Object.freeze({
    handle,
    sourceUrl: `https://x.com/${handle.slice(1)}`,
    internalUse,
    verificationStatus: "user-seed",
    automationAllowed: false,
    evidenceNote: "Evidence missing — source verification required",
